...
begin
  ...
  try
    ...
    Check(S1, ['פûגא', 'qwerty', 'abc', 'zxcvb']);
    if S1.IndexOfObject(Pointer(1)) <> 2 then Error;
    S2.Assign(S1);
    Check(S2, ['פûגא', 'qwerty', 'abc', 'zxcvb']);
               { nil } { 2 }     { 1 }  { nil }
    ...
    S2.Move(0, 3);
    Check(S2, ['qwerty', 'abc', 'zxcvb', 'פûגא']);
    ...
    Check(S1, ['qwerty', 'abc', 'zxcvb', 'פûגא',
               'qwerty', 'abc', 'zxcvb', 'פûגא']);
    S:=S1.Text;
    {$IFDEF V_WIN}
    writeln(AnsiToOem(S));
    {$ENDIF}
